<?php 
	...
	class Evaluacion_mixta 
	{
		...
		public function evaluacion_realizada($id,$codcur,$codpar){
			$sql = "SELECT count(*) as total FROM alumno WHERE cod_cur = ? and cod_par = ? AND estado = 1 AND codigo IN (SELECT codalu FROM em_alumno where codeva = ? AND estado = 1 GROUP by codalu);";
			$type = "iii";
		...
		public function evaluacion_no_realizada($id,$codcur,$codpar){
			$sql = "SELECT count(*) as total FROM alumno WHERE cod_cur = ? and cod_par = ? AND estado = 1 AND codigo NOT IN (SELECT codalu FROM em_alumno where codeva = ? AND estado = 1 GROUP by codalu);";
			$type = "iii";
			$params = array($codcur,$codpar,$id);
			$result = ejecutar_consulta($this->db,$sql,$type,$params);
			return $result;
