	...
	class Justificacion_decidir
	{
		...
		public function save($id_ser_decidir,$justificacion,$fechareg,$codprof){
			if($id = $this->existeRegistro($id_ser_decidir)){
				$sql = "UPDATE justificacion_decidir SET justificacion = ?,fechareg = ?, codprof = ? WHERE id = ?";
				$type = "sssi";
				...
				$sql = "INSERT INTO justificacion_decidir(id_ser_decidir,justificacion,fechareg,codprof,estado) 
		  					   					   VALUES(?,?,?,?,1)";
				$type = "isss";
		...
		public function get_justificacion($id){
			$sql = "SELECT * FROM justificacion_decidir WHERE id_ser_decidir = ? AND estado = 1";
			$type = "i";
			$params = array($id);
