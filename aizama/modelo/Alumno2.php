...
/*       if (!empty($this->pago)) {
        $this->existePago() 
          ? $this->updateKardexGastosOperativos($this->pago)
          : $this->insertKardexGastosOperativos($this->pago);
      } */
    ...
    public function insertLibro($pago, $nrecibo) {
      ...
      $hora = date("H-i-s");
      $query = "INSERT INTO libro(fecha, codigo, nombre, nreci, debe, cambio, tipo, hora) 
                VALUES ('{$fecha}', '{$this->codigo}', '{$nombre}', '{$nrecibo}', '{$pago}', '6.90', 'r', '{$hora}')";
      $db->query($query);
      $db->close();
    }
    ...
    public function insertNumeroFacturaGastoOperativo() {
      $nrecibo = 0;
      $db = connectDB();
      $stmt = $db->prepare("SELECT insert_recibo_gasto_operativo()");
      $stmt->execute();
      $stmt->bind_result($nrecibo);
      $stmt->fetch();
      $stmt->free_result();
      $stmt->close();
      $db->close();
