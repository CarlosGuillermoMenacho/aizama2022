...
//		clearInputs();
		...
		if(parseInt(param)==param)  //ESTO VERIFICA QUE EL DATO SEA NUMERO
		{
			...
			{
				//alert('sacar notas 1');
				sacar_notas(param,$('#vf_curso').val());
				return true;
			}
			else
			{
...
{
//	alert('op 1');
	if (idcurso=='1ro PRIMARIA - A' || idcurso=='1ro PRIMARIA - B' || idcurso=='2do PRIMARIA - A' || idcurso=='2do PRIMARIA - B' || idcurso=='3ro PRIMARIA - A' || idcurso=='3ro PRIMARIA - B' || idcurso=='4to PRIMARIA' || idcurso=='5to PRIMARIA' || idcurso=='6to PRIMARIA')
	{
...
//			var html=$.ajax({type:"GET",url:'get_notas_1_2.php',data:{id:idalumno},async:false}).responseText;
		...
		{
			if (idcurso=='3ro SECUNDARIA' || idcurso=='4to SECUNDARIA' || idcurso=='5to SECUNDARIA' || idcurso=='SEXTO DE SECUNDARIA')
			{
//				alert('op 3-6 secundaria');
				var html=$.ajax({type:"GET",url:'get_notas_3ro.php',data:{id:idalumno},async:false}).responseText;
			}
			else
			{
...
{
//	alert('goles 1');
	var html=$.ajax({type:"GET",url:'get_alumnos_bol.php',data:{id:id},async:false}).responseText;
	if(html=='eNoResults'){
...
//	$('#vf_materno').val(materno);
	$('#vf_nombre').val(nombres+" "+paterno+" "+materno);
	$('#vf_curso').val(curso);
	$('#vf_turno').val(turno);
...
function clearFactura()
{
	...
	tblHeader+='<th>MATERIA</th>';
	tblHeader+='<th>1er. Trimestre</th>';
	tblHeader+='<th>2do. Trimestre</th>';
	tblHeader+='<th>3er. Trimestre</th>';
...
//							   $('#btnSalir').bind('click',function(){location.href='menu_alumno.php';});
