...
function lenguaje()
{
	...
	{
		location.href='ver_practicos.php?vista=2';
	}
}
...
function historia()
{
	...
	if (getAsignarMateria($codm))
	{
		location.href='ver_practicos.php?vista=2?vista=2';
	}
}
function ortografia()
{
...
//	alert ('hola 7');
	if (parseInt(nro_prac)>0){
		var html=$.ajax({type:"POST",url:'get_lista_videos_ver.php?op=lista_llena',data:{materia:codMateria,curso:codCurso},async:false}).responseText;
	}
//	alert ('hola 6.8');
...
function crearTabla(){
    ...
    tabla+='<tr align="center"><td><b>No.</b></td><td align="center"><b>DESCRIPCION DE VIDEOS</b></td><td align="center"><b>VER VIDEO</b></td>';
    tabla+="</tr>";
    
    for(i=0;i<filas;i++){
        tabla+="<tr>";
        tabla+="<td>"+(i+1)+ '</td><td><div id="alum'+(i+1)+'"></div><textarea id="alum'+(i+1)+'" type="text" rows="5" cols="50" MAXLENGTH="250" style="border:0;outline: none;" >'+lista[i]['pregunta']+'</textarea></td><td align="center"><a href="'+lista[i]["imagen"]+'" target="_blank" class="btnVideo">Ver Video</a>'/*<input name="btn19" type="button" class="btn" id="btn19" value="Ver Video" onclick="Ver_cuestionario('+(i+1)+')" />*/+'</td>';
        tabla+="</tr>";
    }
    tabla+="</table>";
...
function CargarVideo(){
	...
	console.log(c_mat);
	var html=$.ajax({type:"POST",url:'get_lista_videos.php',data:{id1:n_curso,id2:c_mat},async:false}).responseText;
	if(html=='Paso2'){
