...
function verPracticos(curso, paralelo, materia) {
    ...
    for(let i = 0; i < listaPracticos.length; i++) {
        ...
        let indicador = listaPracticos[i]['indicador'];
        console.log(limite);
        let check = "";
    ...
    for (var i = 0; i < inputs_indicadores.length; i++) {
        ...
        if( tex_ind != control_indicadores[i][1]){
            ...
            $.post(
                    ...
                    {id:codpract,texto:tex_ind,tipe:1},
                    response => {
...
const mostrar_mensaje = (mensaje,estilo) => {
    if(estilo == "success"){
        ...
        $('#notificacion').addClass("notifications_error");
...
function mostrarAlumnosPracticos() {
    ...
    listaAlumnos.forEach( alumnos => {
        ...
        if (estado === '1') {
            ...
            let checked = "";
            if(editable === '1' )checked = "checked";
            $('#camposAlumnos').append(`<tr>
                <td data-label="Nro">${nro}</td>
                <td class="nombreAlumno" data-label="Alumno">${nombre}</td>
                <td data-label="Pdf">&nbsp;</td>
                <td data-label="Habilitar Edición"><input type="checkbox" onchange="habilitacionEdicion(this, ${id})" ${checked}></td>
                <td data-label="Nota">
                    <div style="display:flex; align-items:center;">
                        <input id="nopres${codalu}" type="number" style="width:70px; margin-right:5px;" value="${nota}">
                        <button onclick="guardarNotaAlumnoNoPresento(${id_pracSelected},${codalu})" style="background-color:teal; padding:5px; color:white; border-radius:5px;">Grabar</button>
                    </div>
                </td>
                <td data-label="Hora">&nbsp;</td>
                <td data-label="Fecha">&nbsp;</td>
                <td data-label="Observación">
                    <div style="display:flex; align-items:center;"><textarea id="obsnopres${codalu}" type="text" style="width:250px; margin-right:5px;height:80px;padding:5px;resize:none;">${observacion}</textarea></div>
                </td>
            </tr>`);
            nro++;
        } else {
            $('#camposAlumnos').append(`<tr>
                ...
                <td data-label="Fecha">&nbsp;</td>
                <td data-label="Observación">
                    <div style="display:flex; align-items:center;"><textarea id="obsnopres${codalu}" type="text" style="width:250px; margin-right:5px;height:80px;padding:5px;resize:none;"></textarea></div>
                </td>
            </tr>`);
            nro++;
        }
...
function guardarNotaAlumnoNoPresento(idPractico,codalu){
    let nota_calificacion = $('#nopres'+codalu).val();
    ...
    Swal.fire({
      ...
      cancelButtonText: 'Cancelar'
        ...
        if (result.isConfirmed) {
          var http = new XMLHttpRequest();
          var url = 'practicos_json.php?op=califpracnopres';
          var nota = document.getElementById(`nopres${codalu}`).value;
          var observacion = document.getElementById(`obsnopres${codalu}`).value;
          var formData = new FormData();
          formData.append('id', idPractico);
          ...
          if (nota > 0 && nota <= 100) {
            ...
            Swal.fire(
            'Error',
            'Ingrese una nota mayor a 0 y menor o igual a 100',
            'error'
            );
            return false;
          }
          ...
          http.onreadystatechange = function() {
              if(http.readyState == 4 && http.status == 200) {
                  if(http.responseText == 'ok') {
                    ...
                    //let checked = document.getElementById(`${codalu}`).parentElement.parentElement.parentElement.children[3].children[0].checked = false;
                    enviarMensaje(codalu,nota,observacion);
                    Swal.fire(
                      'Éxito nota registrada',
                      'Presione ok para salir',
                      'success'
                      );
                    obtenerEstudiantes(id_pracSelected,nro_practico);
                  } else {
                    Swal.fire(
                      'Error',
...
function obtenerEstudiante(id){
    for (var i = 0; i < listaAlumnos.length; i++) {
        if(listaAlumnos[i].codalu==id)return listaAlumnos[i];
    }
    return [];
}
