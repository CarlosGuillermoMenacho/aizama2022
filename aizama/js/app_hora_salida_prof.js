...
//		alert('2');
		var html=$.ajax({type:"POST",url:'asistencia_prof_json.php?op=asist',data:{tipo:'2'},async:false}).responseText;
//		alert('3');
		if (html=='error') {
		...
		if (html=='noIngreso') {
			alert('Debe registrar su ingreso previamente...!!!');
			return false;
		}
		if (html=='ok') {
		//	$('#codalu').val("");
		...
		//	$('#codalu').focus();
			if (datos_prof.CELPRO != null && datos_prof.CELPRO != "") {
				$.get(
					"controlador/asistencias_profesor_controlador.php?op=getSalidaHoy",
					datos => {
						if (datos.status == "eSession")location.href = "docentes.php";
						if (datos.status == "noData")Swal.fire("No se obtubieron los datos del profesor...\nVuelva a iniciar sesi&oacute;n...");
						if (datos.status == "ok") {
							...
							let mensaje = `${fecha} - ${dia}: Hora de salida ${hora}`;
