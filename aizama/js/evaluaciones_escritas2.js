...
function cargarLista()
{
	...
	$.ajax({
			...
			success:function(r){
				...
				let horai = $('#horaini').val();
				let fechaf = $('#fechafin').val();
				let horaf = $('#horafin').val();
				let descrip = $('#descripcion').val();
				let nota2 = $('#nota_evaluacion').val();
				//alert(3);
		...
		if(evaluado(codalu)){
			...
			$('#campos-lista-alumnos').append(
											`<tr>
												<td data-label="No.">${indice}</td>
												<td data-label="Nombre" style="text-align:left;">${nombre}</td>
												<td data-label="Nota"><input type="number" id="calPres${codalu}" class="input_nota"/></td>
												<td data-label="Opción"><button onclick="calificar_presencial(${codalu},${evaluacion});">Calificar</button></td>
											</tr>`
											);
		}
		indice++;
...
function calificar_presencial(codalu,codeva){
    if($(`#calPres${codalu}`).val()==""){
        Swal.fire('Debe ingresar una nota para calificar');
        return;
    }
    let nota = parseInt($(`#calPres${codalu}`).val());
    if(nota > 0 && nota <= nota_evaluacion){
        
    }
    Swal.fire(`Debe ingresar una nota entre 1 y ${nota_evaluacion}`);
    return;
}
