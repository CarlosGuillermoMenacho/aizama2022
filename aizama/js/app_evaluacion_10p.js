...
function cargarMaterias() {
  ...
  for(var i=0; i< materias.length; i++) {
    ...
    $('.lista-materias').append(
        `<li>
            <button class="info-materia" onclick="mostrarTablaExamen(\'${codigoMateria}\')">
                <div class="materia">
...
function verExamen(codigoExamen,n_eva) {
  var html=$.ajax({type:"GET",url:'asignar_examen_10p.php',data:{id:n_eva},async:false}).responseText;
  if(html=='eNoNumeric'){
  ...
  if(html=='eHecho'){
    ...
    //    alert('holassss');
    location.href='advertencia_10p.php';
    return true;
  }
...
function mostrarTablaExamen(codigoMateria) {
  ...
  /* Asigno la materia */
  var html=$.ajax({type:"GET",url:'asignar_materia_10p.php',data:{id:codigoMateria},async:false}).responseText;

  var count = 0;
  var codigo_materia = codigoMateria;
  $('#campos').children().remove();    
  $('.contenedor-lista').removeClass('active');
  $('.elija').text("elija un examen");  
  ...
  if(cantidad_clase > 0) {
      ...
      for(var i=0; i<evaluaciones.length; i++) {
        if( evaluaciones[i]['codmat'] == codigoMateria ) {
            ...
            let enlace = evaluaciones[i]['enlace'];
                ...
                case '0':
                    $('#campos').append(
                        `<tr>
                          ...
                          <td data-label="Ver" >
                            <button class="link" onclick="verExamen(${codigoExamen},${codeva})">
                    ...
                    case '3':
                      $('#campos').append(
                        `<tr>
                          ...
                          <td data-label="Ver" >
                            <button class="link" onclick="verLive('${enlace}')">
                                <img src="images/icon-examen.svg">
                            <button/>
                          </td>
                          <td data-label="Nota">Sin nota</td> 
...
function obtenerEvaluaciones() {
  ...
  peticion.open('POST', 'evaluaciones_json_10p.php?op=evalu', false);
  peticion.send();
}
