...
function PadronEnc(e,param)
{

	if(isKeyEnter(e))  //presiono enter
	{
		getPadronEnc(param); //verifica si esta en el padron
		getEncargado(param); //verifica si esta en la tabla de Encargados
		return true;
	}
}
...
function grabado(id)
{
	var html=$.ajax({type:"GET",url:'get_grabado_lista.php',data:{id:id},async:false}).responseText;
...
function formSubmit()
{
	if (validarDato())
	{
		if (grabado($('#vf_carnet2').val()))
		{
			...
			$('#vf_otro').val('');
			$('#vf_otro2').val('');
			// no funciona			$('#vf_registro').val('$('#vf_registro').val()');
			$('#vf_carnet2').focus();
			return true;
		}
		else
		{
		if (getOtro($('#vf_otro').val()))
		{
			alert('ESTE REGISTRO YA ESTA GRABADO !!! Encargado: ' + $('#vf_otro2').val());
		}
		}
	}
	else
