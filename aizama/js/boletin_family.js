...
function peticionNotas() {
  ...
  peticion.open("POST", "boletin_json.php?op=tutor", false);
  ...
  switch (respuesta["status"]) {
    ...
    case "ok":
      ...
      break;
      case 'noData':
        Swal.fire('No hay datos del alumno');
        d.querySelector(".info-alumno").style.display = "none";

      d.querySelector(".contenedor-tables").style.display = "none";
      break;
      case 'blocked':
        Swal.fire(`<div>No tiene acceso al boletín, comun&iacute;quese con administraci&oacute;n <div><a href="${administracion}" target="_blank"><img src="../images/whatpsapp.svg"></a></div></div>`);
        d.querySelector(".info-alumno").style.display = "none";
...
peticion.onreadystatechange = function () {
  ...
  if (this.readyState === 4 && this.status === 200) {
    ...
    const lista_alumnos = respuesta["alumnos"];

    for (let i = 0; i < lista_alumnos.length; i++) {

      let codigo = lista_alumnos[i]["codalu"];
...
peticion.open("POST", "controlador/practicoDigital_controlador.php?usr=tut&op=get_practicos_tutor", false);
