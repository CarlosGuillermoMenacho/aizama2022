...
function guardarNota(cod_alu,element){
    if(click.includes(cod_alu))return;
    click.push(cod_alu);
    console.log(click);
    let nota = $(`#nota${cod_alu}`).val();
    if(nota==''){
        Swal.fire('Ingrese un valor...');
        return;
    }
    $.post("evaluaciones_json.php?op=guardarNota",
          {codalu:cod_alu,codexa:coddeeva,nota:nota},
          (datos,estado,xhr)=>{
            if (datos == "ok") {
              Swal.fire(
                        'Se registró la nota exitosamente...!!!'
                        );
              preguntasEvaluacion(coddeeva,nro_evaluacion);
            }
          },'text');
}
      ...
      if(nota!=""){
        ...
        $('#camposAlumnos').append(`
            <tr>
              <td data-label="Nro">${nro}</td>
              <td class="alineacion-alumno" data-label="Alumno">${nombre}</td>
              <td data-label="Nota"><input type="number" class="input2" id="nota${codalu}"/></td>
              <td data-label="Revisar"><button class="btnSave" onclick="guardarNota(${codalu},this)">GUARDAR</button></td>
